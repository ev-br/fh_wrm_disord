...
!-------------- read the files ----------------
	...
	allocate( g_uu(nfls,0:100), g_ud(nfls,0:100) )
...
!--------  do density-density correlator ----------------
	allocate( g_uu_all(0:100), g_ud_all(0:100) )

	Z_all = sum(Z(:))

	do i=0,N(1,1)/2; 
	  g_uu_all(i)=sum ( g_uu(:,i) ) / nfls / Z_all
	  g_ud_all(i)=sum ( g_ud(:,i) ) / nfls / Z_all
	enddo

	open(1,file='g_uu'//trim(fsuffix)//'.dat')
	open(2,file='g_ud'//trim(fsuffix)//'.dat')
	  do i=0,N(1,1)/2
	       write(1,*)i,g_uu_all(i)
	       write(2,*)i,g_ud_all(i)
	  enddo
	close(1); close(2)
...
! ------------  where to start from --------------------
	...
	do i=1,nfls
	  istart(i,:)=(/ 1,2,b_n(i)/4,b_n(i)/2 /)     ! everyth, drop 1st block, drop 1st quarter
	enddo
...
! -------------  glue all the files -----------------
	...
	do i=1,nfls;  
	  ...
	  do j=istart(i,klm),b_n(i)
	     i_all = i_all + 1
	     PE_all(i_all) = PE_stat(i,j)
	     KE_all(i_all) = KE_stat(i,j)
	     im_all(i_all) = im_stat(i,j)
...
!--- pot. energy -----------------------------------
	call bSTAT(PE_all(1:b_n_all),b_n_all,1,av,err)
...
!--- kin. energy -----------------------------------
	call bSTAT(KE_all(1:b_n_all),b_n_all,1,av,err)
...
!--- ndens
	call bSTAT(ndens_all(1:b_n_all),b_n_all,1,av,err)
...
c	kf = (3.d0*pi*pi*av)**(1.d0/3.d0)
c	ef = kf**2/2.d0                          ! m=1
c	
c	def = ef*(err/av)*(2.0/3.0)
c
c	print 777, ef,def, kf
c 777  format(8x,'E_F =',g12.5,4x,' +/- ',g12.5,' k_F = ',g12.5 )


!--- integrated correlator ---------------------------

c	call bSTAT(im_all(1:b_n_all),b_n_all,1,av,err)
c	print*,' '
c      print 703, av, err
c 703  format(8x,'g_im(w=0,k=0) =',g12.5,4x,' +/- ',g12.5)
	...
	deallocate(PE_all,KE_all,ndens_all,im_all, empty)
...
!---------------------
	...
	open(1,file=trim(fname))
	  ...
	  read(1,*)cvoid
	  read(1,*)g_uu(i,0:N(i,1)/2)
	  read(1,*)g_ud(i,0:N(i,1)/2)
	close(1)
